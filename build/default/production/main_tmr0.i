...
CONFIG_RELOJ:
    ...
    BSF OSCCON, 5 ; ((OSCCON) and 07Fh), 6
...
CONFIG_TMR0:
    BANKSEL OPTION_REG
    BCF ((OPTION_REG) and 07Fh), 3 ; prescaler a TMR0 (NOTA: En clase olvidamos asignar el Prescaler al TMR0)
    BSF ((OPTION_REG) and 07Fh), 2
    BSF ((OPTION_REG) and 07Fh), 1
    BSF ((OPTION_REG) and 07Fh), 0 ; prescaler 1 : 256
    BCF ((OPTION_REG) and 07Fh), 5 ; Incremento con pulsos del reloj interno
...
REINICIO_TMR0:
    ...
    MOVLW 61 ; 50ms
    MOVWF TMR0 ; Cargamos valor inicial
    BCF ((INTCON) and 07Fh), 2 ; Limpiamos bandera
    return

CONFIG_INT:
    BANKSEL INTCON
    BSF ((INTCON) and 07Fh), 7 ; Habilitamos interrupciones globales
    BSF ((INTCON) and 07Fh), 5 ; Habilitamos interrupciones del TMR0
    BCF ((INTCON) and 07Fh), 2 ; Limpiamos bandera
    RETURN
